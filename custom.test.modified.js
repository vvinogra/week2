...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
    ...
    // Alice deposits into tornado pool in L1
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    ...
    const aliceAmountAfterSendingToBob = aliceDepositAmount.sub(bobSendAmount)
    const bobSendUtxo = new Utxo({ amount: bobSendAmount, keypair: Keypair.fromString(bobAddress) })
    const aliceBobTransferChangeUtxo = new Utxo({
      amount: aliceAmountAfterSendingToBob,
      keypair: aliceDepositUtxo.keypair,
    })
    await transaction({ tornadoPool, inputs: [aliceDepositUtxo], outputs: [bobSendUtxo, aliceBobTransferChangeUtxo] })
    ...
    // Alice withdraws all her funds from the shielded pool
    const aliceWithdrawAmount = aliceAmountAfterSendingToBob
    const aliceEthAddress = '0xdEAd10000000000000000000000000000000beEf'
    const aliceWithdrawChangeUtxo = new Utxo({
      amount: aliceAmountAfterSendingToBob.sub(aliceWithdrawAmount),
      keypair: aliceKeypair,
    })
    await transaction({
      tornadoPool,
      inputs: [aliceBobTransferChangeUtxo],
    ...
    expect(bobEthAddressBalance).to.be.equal(bobWithdrawAmount)
  })
})
